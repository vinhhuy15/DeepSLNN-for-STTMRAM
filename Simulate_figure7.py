...
def simulate_slnn_point(sigma_mu: float) -> tuple[float, float]:
    """Chạy riêng SLNN cho một điểm sigma để backfill hoặc cập nhật theo model mới."""
    sln_bit = 0
    sln_frm = 0
    sln_bits_tot = 0
    sln_frm_tot = 0
...
# ====================== ĐỒNG BỘ KẾT QUẢ SLNN THEO MODEL MỚI ======================
needs_slnn_refresh = (
...
if needs_slnn_refresh:
    print("  Phát hiện model SLNN mới hoặc dữ liệu SLNN cũ thiếu, đang tính lại đường SLNN...")
    ckpt["BER_slnn"] = []
    ckpt["FER_slnn"] = []
